...
export const isMatchCountryAndSearchTerm = (c: Country, term: string): boolean => {
  ...
  return (
      (c.cca3 && c.cca3.toUpperCase().includes(aux)) ||
      (c.ccn3 && c.ccn3.toUpperCase().includes(aux)) ||
      (c.cca2 && c.cca2.toUpperCase().includes(aux)) ||
      (c.cioc && c.cioc.toUpperCase().includes(aux)) ||
      (c.name && c.name && c.name.common.toUpperCase().includes(aux)) ||
      (c.name && c.name && c.name.official.toUpperCase().includes(aux))
  );
}